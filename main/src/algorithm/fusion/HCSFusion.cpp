/********************************************************************
	created:	2016/03/11
	created:	11:3:2016   10:38
	filename: 	D:\code\vs2010\C\Fusion\HCSFusion.cpp
	file path:	D:\code\vs2010\C\Fusion
	file base:	HCSFusion
	file ext:	cpp
	author:		YS
	
	purpose:	HCSFusion类实现
	HCS融合算法
*********************************************************************/

#include "HCSFusion.h"

void HCSFusion::MeanStd_HCS_Fusion(const char* Input_PAN_FileName, const char* Input_MS_FileName, const char* Output_MS_FileName,const char* LogName,int* bandlist,int InterpolationMethod){
	/*
	 *融合方法：HSI
	...
	try{
		...
		////PAN影像与MS影像融合：用灰度拉伸后的图像替换I分量生成新HSI
		//for (i = 0; i < COLS; i++)
		//	DATA2D(New_MSData, 0, i, COLS) = (PANData[i]) ; //不需要把PAN图像转换到[0,1]
