...
#ifndef _FDCT_WRAPPING_INLINE_HPP_
#define _FDCT_WRAPPING_INLINE_HPP_

#include "fdct_wrapping_inline.hpp"

FDCT_WRAPPING_NS_BEGIN_NAMESPACE

inline int fdct_wrapping_fftshift(CpxNumMat& T, CpxOffMat& O)
{
...
inline int fdct_wrapping_ifftshift(CpxOffMat& O, CpxNumMat& T)
{
...
inline int fdct_wrapping_window(double x, double& l, double& r)
{
  if(x<=0) {
	l = 0;	 r = 1;
  } else if(x>=1) {
...
inline int fdct_wrapping_rangecompute(double XL1, double XL2, int& XS1, int& XS2, int& XF1, int& XF2, double& XR1, double& XR2)
{
...
inline int fdct_wrapping_lowpasscompute(double XL1, double XL2, DblOffMat& lowpass)
{
  ...
  for(int i=-XF1; i<-XR1/2; i++) {
	 ...
	 double l,r; fdct_wrapping_window(x, l, r);
	 lowpass1(i) = l;
	 lowpass1(-i) = l;
  }
  //cerr<<lowpass1;
...
template <class F>
inline int fdct_wrapping_rotate_forward(int f, F XL1, F XL2, F& TL1, F& TL2)
{
  if(f==0) { //time 1
	 TL1 = XL1;	 TL2 = XL2;
  } else if(f==1) { //time -i 
...
template <class F>
inline int fdct_wrapping_rotate_backward(int f, F XL1, F XL2, F& TL1, F& TL2)
{
  if(f==0) {
	 TL1 = XL1;	 TL2 = XL2;
  } else if(f==1) {
...
template <class F>//inline int fdct_wrapping_rotate_forward(int f, CpxOffMat& X, CpxOffMat& T)
inline int fdct_wrapping_rotate_forward(int f, OffMat<F>& X, OffMat<F>& T)
{
...
template <class F>//inline int fdct_wrapping_rotate_backward(int f, CpxOffMat& X, CpxOffMat& T)
inline int fdct_wrapping_rotate_backward(int f, OffMat<F>& X, OffMat<F>& T)
{
  int XS1 = X.m();  int XS2 = X.n();
  int XF1 =-X.s();  int XF2 =-X.t();
  int TS1, TS2;
